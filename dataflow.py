...
def voxceleb2_preproces(data: str,args:dict):
    id,start,end,x,y=[0,3,10,256,256]
    filename=data.rsplit(".mp4")[0]
    os.makedirs(filename,exist_ok=True)
    path=os.path.join(f"tmp/video",filename)
    subprocess.call(f"ffmpeg -i {data} -ss {start} -to {end} {filename}/%5d.png",shell=True,stdout=DEVNULL,stderr=DEVNULL)
    subprocess.call(f"ffmpeg -i {data} -ss {start} -to {end} {filename}/audio.wav",shell=True,stdout=DEVNULL,stderr=DEVNULL)
    return [filename]
...
if __name__=="__main__":
    ...
    for  i,item in enumerate(job_list):
        try:
            if "https://" in item and parameters_list[i]["is_download"]:
                raw_path=download(item,parameters_list[i])
            else:
                raw_path=item
            if parameters_list[i]["is_avspeech"]:
                video_dir_list=avspeech_preprocess(raw_path,parameters_list[i])
            else:
                video_dir_list=voxceleb2_preproces(raw_path,parameters_list[i])
            for video_dir in video_dir_list:
                ...
                if parameters_list[i]["valid"]:
                    np.savez(f"{video_dir}",landmark=landmark_list,bbox=bbox_list,angle=angle_list)
                    move_data(video_dir,"currect/")
